function [radii,center,diameters,evecs,z_res,u,Plotting_vectors, diameters_save] = Ellipsoid_Fitting_fxn(n,image,xy_res,z_res_OG,theta_n,phi_n,r_n,run, path,save_filename)
...
hold on
scatter3(Data_mat(:,1),Data_mat(:,2),Data_mat(:,3),'marker','.','sizedata', 200);
h = surf(x_new,y_new,z_new);
axis equal
alpha(0.3)
grid on
...
ellipsoid_surface = isosurface(x,y,z,Ellipsoid,1); 
p = patch(isosurface(x,y,z,Ellipsoid,1)); 
set(p,'FaceColor','g','EdgeColor','none'); 
alpha(p,0.3)
...
if Save_coords == 1; 
    ...
    for n = 1:36
        ...
        diameters_save=diameters(:,1)
        save([path,save_filename,'_bead_',num2str(bead_n),'_run',num2str(run),'_',date, '.mat'], 'save_coords','diameters_save','ellipsoid_surface','center')
        %writematrix(save_coords,strcat('Bead_',string(bead_n),'_run',string(run),'.csv'))
        
        bead_name=['_bead_',num2str(bead_n)];
        savefig(fig_points,[path,save_filename,bead_name,'_run',num2str(run),'_',date,'_pointcloud.fig']);
        saveas(fig_ellipsoid,[path,save_filename,bead_name,'_run',num2str(run),'_',date,'_ellipsoid.fig']);
        
        
else
     diameters_save=[NaN;NaN;NaN];
end
