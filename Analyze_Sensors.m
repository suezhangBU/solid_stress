...
for n = 1:length(rois)
   ...
   [radii,center,diameters,evecs,z_res,u,Axes_points] = ...
       Ellipsoid_Fitting_fxn2(n,subimage,xy_res,z_res_OG,theta_n,phi_n,r_n,run);
...
% bead_number=input('What bead number is this?');
% bead_name=['bead' num2str(bead_number)]
saveas(figure(2),[path,save_filename,bead_name,'pointcloud.fig']);
saveas(figure(3),[path,save_filename,bead_name,'ellipsoid.fig']);
save([path,save_filename,bead_name, '.mat'])
diameters(:,1)
